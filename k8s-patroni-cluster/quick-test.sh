...
if [ $pg_ready -eq 3 ]; then
    log_info "PostgreSQL 集群健康 ($pg_ready/3 节点)"
else
    log_warn "PostgreSQL 集群部分健康 ($pg_ready/3 节点)"
fi
...
# 测试4: 检查 HAProxy
echo ""
echo "测试 4: 检查 HAProxy..."
if kubectl get deployment "$CLUSTER_NAME-haproxy" -n "$NAMESPACE" &>/dev/null; then
    replicas=$(kubectl get deployment "$CLUSTER_NAME-haproxy" -n "$NAMESPACE" -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
    if [ "$replicas" -gt 0 ]; then
        log_info "HAProxy 运行正常 ($replicas 个副本)"
    else
        log_error "HAProxy 无可用副本"
    fi
else
    log_error "HAProxy 部署不存在"
fi

# 测试5: 检查服务
echo ""
echo "测试 5: 检查服务..."
services=("$CLUSTER_NAME-etcd" "$CLUSTER_NAME-postgresql" "$CLUSTER_NAME-haproxy")
...
kubectl port-forward -n "$NAMESPACE" svc/"$CLUSTER_NAME-haproxy" 15432:5000 >/dev/null 2>&1 &
PORT_FORWARD_PID=$!
...
if [ $etcd_ready -ge 2 ] && [ $pg_ready -ge 2 ] && [ "$leader_found" = true ]; then
    ...
    echo ""
    echo "📋 连接信息:"
    echo "• 主库连接 (读写): kubectl port-forward -n $NAMESPACE svc/$CLUSTER_NAME-haproxy 5432:5000"
    echo "• 副本连接 (只读): kubectl port-forward -n $NAMESPACE svc/$CLUSTER_NAME-haproxy 5433:5001"
    echo "• HAProxy 统计页面: kubectl port-forward -n $NAMESPACE svc/$CLUSTER_NAME-haproxy 7000:7000"
    echo "• 默认用户名: postgres"
